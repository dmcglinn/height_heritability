...
par(mfrow=c(1,1))
hist(dat$offspring.height[dat$semester=='fall' & dat$year ==2023],
     xlab='Student Heights (in)', main='')
ylims = range(dat$offspring.height)
xlims = range(dat$mid.parent, na.rm=T)
plot(offspring.height ~ mid.parent, data=dat, subset=semester=='fall' & year==2023,
     pch=19, xlim=xlims, ylim=ylims, main='Fall 2023')
abline(lm(offspring.height ~ mid.parent, data=dat, subset=semester=='fall' & year==2023))
###
plot(offspring.height ~ jitter(mid.parent), data=dat, pch=19, xlim=xlims, ylim=ylims,
     main='Fall 2014-Fall 2023')
